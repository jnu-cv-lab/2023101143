...
1. 随着 nfeatures 从 500 增加到 1000、2000，模板图和场景图检测到的关键点数量增加，匹配数量也随之从 149 增加到 287 和 511。说明增大特征点数量可以获得更多的候选匹配点。
2. RANSAC 内点比例分别为 0.215、0.181、0.129，可以看到随着特征点数量增加，内点比例反而下降。这说明虽然匹配点数量增多，但错误匹配的比例也在增加，导致整体匹配质量下降。
3. 不是。特征点数量增加虽然能提供更多匹配候选，但也会引入更多来自背景或重复纹理的低质量特征点，从而增加错误匹配的概率，降低匹配的几何一致性。本实验中 nfeatures=500 和 1000 能够成功定位，而 nfeatures=2000 定位失败，说明特征点并非越多越好，合理的特征点数量更有利于稳定定位。
